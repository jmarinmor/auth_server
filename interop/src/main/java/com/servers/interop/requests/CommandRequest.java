...
public class CommandRequest<T> {
    ...
    public boolean containsCommand() {
        return command != null || commandEncoded != null;
    }
        ...
        if (publicKey == null) {
            try {
                if (commandEncoded != null) {
                    String json = Crypter.base64StringToString(commandEncoded);
                    ret = serializer.fromJson(json, aClass);
                } else
                    ret = command;
            } catch (Exception e) {
                e.printStackTrace();
            ...
            try {
                ...
                if (commandEncoded != null) {
                    byte[] bytes = Crypter.base64StringToBytes(commandEncoded);
                    byte[] decrypted = decrypter.crypt(bytes);
                    String json = Crypter.bytesToString(decrypted);
                    ret = serializer.fromJson(json, aClass);
                } else {
